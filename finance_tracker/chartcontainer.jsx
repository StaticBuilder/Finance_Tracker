  ...
  const renderChart = () => {
    switch (type) {
      case 'pie':
        ...
        case 'line': 
          // Calculate the maximum value for both budget and spend
          const maxBudgetValue = Math.max(
            ...data.map(item => parseFloat(item.amount) || 0),
            ...data.map(item => item.totalSpend || 0)
          );
          ...
          return (
            <LineChart data={dataType === 'budget' ? data : comparisonData}>
              <XAxis dataKey="name" />
              <YAxis 
                domain={[0, dataType === 'budget' ? maxBudgetValue * 1.1 : 'auto']} 
                tickFormatter={(value) => {
                  if (value >= 1000) {
              ...
              <Line
                ...
                strokeWidth={2}
                name={dataType === 'budget' ? "Budget" : labels[0]}
              />
              <Line
                type="monotone"
                ...
                strokeWidth={2}
                name={dataType === 'budget' ? "Spent" : labels[1]}
              />
            </LineChart>
          );
